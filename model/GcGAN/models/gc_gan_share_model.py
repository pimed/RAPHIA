...
class GcGANShareModel(BaseModel):
    ...
    def affine(self, tensor, param):
        param = param.view(-1, 2, 3)
        grid = F.affine_grid(param, tensor.size())
        grid = grid.float()
        grid = grid.cuda()
        tensor = F.grid_sample(tensor, grid, mode = 'nearest', padding_mode='border')
        return tensor
    ...
    def forward(self):
        ...
        ### generate several random affine transformations
        aff_trans = np.zeros((self.input_A.size()[0],6))
        for i in range(0,self.input_A.size()[0]):
            angle = np.random.uniform(-np.pi,np.pi)
            shear_x = np.random.uniform(-0.05,0.05)
            shear_y = np.random.uniform(-0.05,0.05)
    
            trans_x = np.random.uniform(-1,1)/10
            trans_y = np.random.uniform(-1,1)/10
            ...
            aff_trans[i][0] = A[0][0]
            aff_trans[i][1] = A[0][1]
            aff_trans[i][2] = trans_x
            aff_trans[i][3] = A[1][0]
            aff_trans[i][4] = A[1][1]
            aff_trans[i][5] = trans_y
        self.theta = torch.from_numpy(aff_trans)
        ...
        elif self.opt.geometry == 'affine':
            ## apply random affine transformation
            self.real_gc_A = self.affine(input_A, self.theta)
            self.real_gc_B = self.affine(input_B, self.theta)
        else:
          raise ValueError("Geometry transformation function [%s] not recognized." % opt.geometry)
          
          
    def get_gc_affine_loss(self, AB, AB_gc, theta):
        loss_gc = 0.0
        AB_gc_gt = self.affine(AB.clone().detach(), self.theta)
    ...
    def backward_G(self):
        ...
        if self.opt.geometry == 'rot':
            ...
            loss_gc = self.get_gc_affine_loss(fake_B, fake_gc_B, self.theta)
    ...
    def test(self):
        ...
        if self.opt.geometry == 'rot':
          ...
          for i in range(0,self.input_A.size()[0]):
              ...
              aff_trans[i][4] = A[1][1]
              aff_trans[i][5] = trans_y
          aff_trans = torch.from_numpy(aff_trans)
          self.real_gc_A = self.affine(input_A, aff_trans)
          self.real_gc_B = self.affine(input_B, aff_trans)
        else:
          raise ValueError("Geometry transformation function [%s] not recognized." % opt.geometry)
